    ...
    <Card className={`min-w-[280px] max-w-[400px] border-2 bg-card transition-all ${getStatusColor(status, selected)}`}>
      <div className="flex items-center gap-3 border-b border-border px-4 py-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground">Route based on a decision</p>
        </div>
      </div>
      ...
      <div className="space-y-2 p-4">
        <div className="text-xs">
          ...
          <div className="mt-1 rounded bg-secondary p-2 text-xs text-foreground">
            {data.condition ? "Ready vs not ready" : "Decision is set by the team"}
          </div>
        </div>
        {status === "running" && (
