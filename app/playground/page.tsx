...
export default function PlaygroundPage() {
  ...
  const [systemPrompt, setSystemPrompt] = useState("You are a helpful market assistant.")
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Unable to reach the market assistant.")
      const data = await res.json()
      const responseText = typeof data?.content === "string" ? data.content : "No response available."
      await simulateStreaming(responseText)
    } catch (error) {
      const message = error instanceof Error ? error.message : "Unable to reach the market assistant."
  ...
  const generateCodeSnippet = () => {
    return `import Entrestate from '@entrestate/sdk';

const client = new Entrestate({
  apiKey: 'YOUR_API_KEY'
});
...
console.log(response.choices[0].message.content);`
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main id="main-content" className="flex-1 pt-20 pb-4 flex flex-col min-h-[calc(100vh-5rem)]">
